...
async function renderImage(
   ...
   * Drawing onto a white background prevents transparent PNGs
...
async function findBestResult(
  ...
  /*
   * Prevent unnecessarily expensive canvas operations for
  ...
  /*
   * We progressively reduce dimensions.
   *
   * For JPEG/WebP we binary-search compression quality at each
   * dimension so we can get as close to 200 KB as practical.
   */
  const scales = [
    1,
    0.96,
  ...
  /*
   * PNG is lossless. There is no useful quality parameter for
   * PNG, so dimensions are reduced until the target is reached.
   */
  if (type === "image/png") {
    ...
    for (const scale of scales) {
      ...
      /*
       * Binary search gives us a much better quality/file-size
       * balance than simply dropping quality in large steps.
       */
      for (
        let iteration = 0;
        iteration < 10;
      ...
      if (bestUnderTarget) {
        /*
         * Prefer the largest result that remains under 200 KB.
         * Larger generally means better visual quality.
         */
        if (
          !bestBlob ||
        ...
        /*
         * If we're already very close to 200 KB, further
         * processing is unlikely to provide a meaningful benefit.
         */
        if (
          TARGET_BYTES -
            bestUnderTarget.size <
   ...
   * JPEG fallback.
   *
   * This is useful when a PNG or another source image cannot
   * efficiently reach the 200 KB target in its original format.
   */
  if (
    !bestBlob ||
    bestBlob.size >
...
export default function ResizeImageTo200KbClient() {
  const fileInputRef =
  ...
  const processFile = useCallback(
    async (selectedFile: File) => {
      ...
      try {
        ...
        /*
         * Allow React to update the UI before the heavier canvas
         * processing begins.
         */
        await new Promise(
          (resolve) =>
  ...
  const handleDownload = () => {
    ...
    link.download = `${baseName}-200kb.${extension}`;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Optimize your image toward 200 KB
                </p>
              </div>
            </div>
          ...
          {!file ? (
            ...
            >
              ...
              <h2 className="mt-6 text-2xl font-bold text-slate-950">
                Upload an image to resize to 200 KB
              </h2>

              <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
                Drag and drop your image here, or choose a file from
                your device. JPG, PNG and WebP are supported.
              </p>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-white/75 p-6 backdrop-blur-sm">
                        <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-xl">
                          ...
                          <p className="mt-1 text-xs leading-5 text-slate-500">
                            Finding the best balance between image
                            quality and the 200 KB target.
                          </p>
              ...
              <aside className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
                <div>
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-500">
                    The tool automatically works toward a maximum file
                    size of 200 KB.
                  </p>
                </div>
                ...
                {showAdvanced && (
                  <div className="mt-3 rounded-xl border border-slate-200 bg-white p-4">
                    <p className="text-xs leading-5 text-slate-500">
                      ...
                      when your main requirement is a 200 KB upload limit.
                    </p>
                  </div>
                )}
                ...
                <div className="mt-6 space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-xs leading-5 text-slate-500">
                      Designed around a 200 KB target
                    </p>
                  </div>
                </div>
              </aside>
